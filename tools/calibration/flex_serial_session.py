...
from flex_session_writer import FlexCsvSessionWriter, make_session_dir


MessageCallback = Callable[[FlexSample | LabelEvent], None]
...
class FlexSerialSession:
    def __init__(self, port: str, baud: int, base_output_dir: str | Path):
        ...
        self.writer: FlexCsvSessionWriter | None = None
    ...
    def start(self):
        ...
        self.writer = FlexCsvSessionWriter(self.session_dir)
        self.serial_port = serial.Serial(self.port, baudrate=self.baud, timeout=0.2)
        time.sleep(2.0)
        self.send_command("stream flex")
    ...
    def _reader_loop(self):
        ...
        while not self._stop_event.is_set():
            ...
            elif parse_runtime_line(line) is not None or line.startswith("POT_SAMPLE,") or line.startswith("IMU_SAMPLE,"):
                continue
            else:
                self._emit_log(line)

    def _emit_message(self, message: FlexSample | LabelEvent):
