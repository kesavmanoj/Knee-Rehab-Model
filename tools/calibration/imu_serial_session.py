...
from imu_session_writer import ImuCsvSessionWriter, make_session_dir
from runtime_serial_protocol import parse_runtime_line


MessageCallback = Callable[[ImuSample | LabelEvent], None]
...
class ImuSerialSession:
    def __init__(self, port: str, baud: int, base_output_dir: str | Path):
        ...
        self.writer: ImuCsvSessionWriter | None = None
    ...
    def start(self):
        ...
        self.writer = ImuCsvSessionWriter(self.session_dir)
        self.serial_port = serial.Serial(self.port, baudrate=self.baud, timeout=0.2)
        time.sleep(2.0)
        self.send_command("stream imu")
    ...
    def _reader_loop(self):
        ...
        while not self._stop_event.is_set():
            ...
            elif parse_runtime_line(line) is not None or line.startswith("FLEX_SAMPLE,") or line.startswith("POT_SAMPLE,"):
                continue
            else:
                self._emit_log(line)

    def _emit_message(self, message: ImuSample | LabelEvent):
