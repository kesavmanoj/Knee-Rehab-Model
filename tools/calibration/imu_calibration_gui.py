...
from imu_calibration_analysis import AnalysisResult, analyze_csv_logs
from imu_serial_protocol import ImuSample, LabelEvent
from imu_serial_session import ImuSerialSession, list_serial_ports
...
class ImuCalibrationGui:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Dual IMU Calibration Tool")
        self.root.geometry("1040x780")
        self.root.minsize(900, 640)

        self.message_queue: queue.Queue = queue.Queue()
        self.session: ImuSerialSession | None = None
        ...
        self.imu_raw_knee_var = tk.StringVar(value="-")
    ...
    def _build_ui(self):
        ...
        status = ttk.LabelFrame(main, text="Live Dual IMU Reading", padding=10)
        ...
        self._add_reading_row(status, 2, "Master IMU", self.master_imu_var)
        self._add_reading_row(status, 3, "Slave IMU", self.slave_imu_var)
        self._add_reading_row(status, 4, "Raw Knee Angle", self.imu_raw_knee_var)
        self._add_reading_row(status, 5, "Mapped IMU Angle", self.imu_angle_var)
        self._add_reading_row(status, 6, "Active Label", self.label_var)
        self._add_reading_row(status, 7, "Samples Logged", self.sample_count_var)

        plot_frame = ttk.LabelFrame(main, text="Live IMU Knee Trace", padding=10)
        plot_frame.pack(fill="x", pady=(12, 0))
        self.trace_canvas = LiveTraceCanvas(plot_frame, y_min=0.0, y_max=145.0)
        ...
        ttk.Button(custom_frame, text="IMU Zero", command=lambda: self._send_command("z")).pack(side="left", padx=(12, 0))

        analysis = ttk.LabelFrame(main, text="Analysis", padding=10)
        analysis.pack(fill="x", pady=(12, 0))
        self._add_reading_row(analysis, 0, "IMU Mapping", self.mapping_var)
        self._add_reading_row(analysis, 1, "Summary", self.summary_var)
    ...
    def start_session(self):
        ...
        self.session = ImuSerialSession(self.port_var.get(), baud, self.output_dir_var.get())
    ...
    def _handle_message(self, message: ImuSample | LabelEvent):
        if isinstance(message, ImuSample):
            self.latest_sample = message
            self.master_imu_var.set(f"{message.master_imu_deg:.2f} deg")
            self.slave_imu_var.set(f"{message.slave_imu_deg:.2f} deg")
            self.imu_raw_knee_var.set(f"{message.imu_raw_knee_angle_deg:.2f} deg")
            self.imu_angle_var.set(f"{message.imu_angle_deg:.2f} deg")
            ...
            self.trace_canvas.add_sample(message.imu_raw_knee_angle_deg)
