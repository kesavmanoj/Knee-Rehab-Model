...
from pot_calibration_analysis import AnalysisResult, analyze_csv_logs
...
class PotCalibrationGui:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Potentiometer Calibration Tool")
        self.root.geometry("1040x780")
        self.root.minsize(900, 640)

        self.message_queue: queue.Queue = queue.Queue()
        self.session: PotSerialSession | None = None
        self.latest_sample: PotSample | None = None
        ...
        self.mapping_voltage_var = tk.StringVar(value="-")
    ...
    def _build_ui(self):
        ...
        status = ttk.LabelFrame(main, text="Live Potentiometer Reading", padding=10)
        ...
        self._add_reading_row(status, 5, "Mapped POT Angle", self.pot_angle_var)
        ...
        plot_frame = ttk.LabelFrame(main, text="Live POT Trace", padding=10)
        ...
        self._add_reading_row(analysis, 1, "Voltage Mapping", self.mapping_voltage_var)
    ...
    def start_session(self):
        ...
        self.session = PotSerialSession(self.port_var.get(), baud, self.output_dir_var.get())
    ...
    def _handle_message(self, message: PotSample | LabelEvent):
        if isinstance(message, PotSample):
            ...
            self.pot_angle_var.set(f"{message.pot_angle_deg:.2f} deg")
